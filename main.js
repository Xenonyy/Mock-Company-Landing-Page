window.onload = () => {
    ...
    if (isMobile === true) {
        $('#phone-nav').css("display", "flex");
        $('#desktop-nav').css("display", "none");
    } else {
        $('#phone-nav').css("display", "none");
        $('#desktop-nav').css("display", "flex");
    }

    // DESKTOP
    for (const item of arr) {
        $(`#desktop-menu-title${item}`).click(() => {
            $(`#desktop-menu-items${item}`).toggleClass("flex");
            $(`#desktop-arrow${item}`).toggleClass("active");
        });
    }

    // Adjust navbar menu width when text becomes bold upon hovering
    (function($) {
        $(function() {
          $('.desktop-menu-items > span').each(function(){
            wid = $(this).width();
